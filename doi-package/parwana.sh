...
#
# parwana.sh — Parwana v1: attested authorization for irreversible actions.
...
#
#   parwana.sh --subject NAME --digest SHA256 --action mint|push --target STR \
...
if [ -z "$CONFIRM" ]; then
  echo "── Parwana: authorization required ──────────────────────────────"
...
mkdir -p "$RCPTS"
STMT="$RCPTS/parwana-${DIGEST:0:12}-${ACTION}.intent.json"
...
RCPT="$RCPTS/parwana-${DIGEST:0:12}-${ACTION}.receipt.json"
python3 - "$RCPT" "$STMT" "$STMT_SHA" "$OTS_FIELD" <<'PY'
import sys,json
rcpt,stmtf,ssha,ots=sys.argv[1:5]
stmt=json.load(open(stmtf))
out={"protocol":"parwana/v1","statement":stmt,"statement_sha256":ssha,
